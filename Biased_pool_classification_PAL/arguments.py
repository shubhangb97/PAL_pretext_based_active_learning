...
def get_args():
    ...
    parser.add_argument('--dataset', type=str, default='svhn', help='Name of the dataset used - cifar10 | cifar100 | svhn')
    ...
    parser.add_argument('--lambda_rot', type=float, default=1, help='weight for self-supervised rotation score')
    parser.add_argument('--val_loss_weightRotation', type=float, default=1, help='Scoring networkvalidation loss weight for rot_loss')
    ...
    parser.add_argument('--samplebatch_size',type=int,default=256,help='Sub query batch size')
    parser.add_argument('--lambda_div', type=float, default=1, help='Weight for diversity')
    parser.add_argument('--biased',type=int,default=2,help='Number of classes to bias')

    args = parser.parse_args()

    if not os.path.exists(args.out_path):
        os.mkdir(args.out_path)
    args.log_name = "accexp"+str(args.lr_rot) + '_' + str(args.lr_task) + '_' + str(args.lambda_kl) + '_'+str(args.lambda_rot) + '_' +  args.optim_task + '_biased' + str(args.biased) + '_' + args.log_name
    return args
